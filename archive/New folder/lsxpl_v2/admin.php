<?php
// lsxpl/admin.php - Secure administration panel for managing LSXPL AI Lab
...
$user_role = $_SESSION['lsxpl_admin_role'] ?? '';
$user_username = $_SESSION['lsxpl_admin_username'] ?? '';
$user_id = $_SESSION['lsxpl_admin_user_id'] ?? 0;
...
if (!in_array($current_tab, $allowed_tabs)) {
    header('Location: admin.php?tab=' . $allowed_tabs[0]);
    exit;
}
$success_message = '';
$error_message = '';
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    try {
        if ($action === 'update_lead_status') {
            ...
            $success_message = "New AI service added successfully.";
            ...
            $success_message = "AI service updated successfully.";
            ...
            $stmt->execute([$id]);
            $success_message = "AI service deleted successfully.";
            ...
            $success_message = "Lab page added successfully.";
            ...
            $success_message = "Lab page updated successfully.";
            ...
            $stmt->execute([$id]);
            $success_message = "Lab page deleted successfully.";
            ...
            $stmt->execute([$id]);
            $success_message = "Research article deleted successfully.";
            ...
            if (isset($_FILES['logo_light']) && $_FILES['logo_light']['error'] === UPLOAD_ERR_OK) {
                if (strpos($_FILES['logo_light']['type'], 'image/') === 0) {
                    move_uploaded_file($_FILES['logo_light']['tmp_name'], __DIR__ . '/logo-light.png');
                } else {
                    $error_message = "Uploaded Light Logo must be a valid image.";
                }
            }
            if (isset($_FILES['logo_dark']) && $_FILES['logo_dark']['error'] === UPLOAD_ERR_OK) {
                if (strpos($_FILES['logo_dark']['type'], 'image/') === 0) {
                    move_uploaded_file($_FILES['logo_dark']['tmp_name'], __DIR__ . '/logo-dark.png');
                } else {
                    $error_message = "Uploaded Dark Logo must be a valid image.";
                }
            }
            ...
            if ($error_message === '') {
                $success_message = "Lab configuration and logo assets updated successfully.";
            }
            ...
            if ($new_pw !== $confirm_pw) {
                ...
                $stmt->execute([$_SESSION['lsxpl_admin_username']]);
                $user = $stmt->fetch();
            ...
            if ($id === (int)$_SESSION['lsxpl_admin_user_id']) {
...
<head>
    ...
    <title>LSXPL AI Lab Admin | Control Panel</title>
...
<body class="admin-body">
    ...
    <div class="admin-sidebar glass-panel">
        <a href="index.php" class="logo">
            <img src="logo-dark.png" alt="LSXPL Logo">
        </a>
        ...
        <ul class="admin-nav">
            ...
            <li class="admin-nav-item <?php echo $current_tab === 'services' ? 'active' : ''; ?>">
                <a href="?tab=services"><i data-lucide="brain" style="width: 18px; height: 18px;"></i> AI Services CRUD</a>
            </li>
            <?php endif; ?>
    ...
    <main class="admin-main">
        <div class="admin-header">
            <div>
                <span class="badge badge-primary">LSXPL AI Lab Console</span>
                <h1 style="font-size: 2rem; margin-top: 0.25rem;">
                    <?php
                    $tab_names = [
                        'reviews' => 'Customer Reviews & Feedback CMS',
                        'leads' => 'AI SaaS Proposals & Inquiries',
                        'services' => 'AI Services & SaaS Systems Portfolio',
                        'industries' => 'AI Industry Solutions (CMS)',
                        'pages' => 'Lab Research Pages (CMS)',
                        'menus' => 'Header Megamenu & Footer Custom Links',
                        'blogs' => 'Insights Publications CRUD',
                        'users' => 'AI Lab User Accounts & Roles',
                        'settings' => 'AI Lab Portal Configurations',
                        'account' => 'Admin Credentials Vault'
                    ];
                    echo isset($tab_names[$current_tab]) ? $tab_names[$current_tab] : 'AI Lab Workspace';
                    ?>
                </h1>
            </div>
            <div style="font-size: 0.9rem; color: hsl(var(--muted-foreground));">
                User: <strong><?php echo htmlspecialchars($_SESSION['lsxpl_admin_username']); ?></strong>
            </div>
        </div>
            ...
            <div class="dash-stats">
                <div class="glass-card dash-stat-card">
                    ...
                    <div>
                        ...
                        <span style="font-size: 0.75rem; color: var(--muted-foreground)">Total SaaS Proposals</span>
                    </div>
                </div>
                <div class="glass-card dash-stat-card" style="border-color: hsla(var(--primary) / 0.3);">
                    ...
                    <div>
                        ...
                        <span style="font-size: 0.75rem; color: var(--muted-foreground)">New Inquiries</span>
                    </div>
                </div>
                <div class="glass-card dash-stat-card" style="border-color: hsla(var(--warning) / 0.3);">
                    ...
                    <div>
                        ...
                        <span style="font-size: 0.75rem; color: var(--muted-foreground)">Active Review</span>
                    </div>
                </div>
            </div>

            <div class="glass-card admin-card">
                <h3 style="font-size: 1.25rem; margin-bottom: 1rem;">Client Proposals</h3>
        ...
        <?php elseif ($current_tab === 'services'): ?>
            ...
            <?php if ($edit_service): ?>
                <div class="glass-card edit-form-panel">
                    <h3 style="font-size: 1.25rem; margin-bottom: 1.5rem;"><i data-lucide="edit"></i> Modify AI Service: <?php echo htmlspecialchars($edit_service['title']); ?></h3>
            ...
            <?php else: ?>
                <div class="glass-card edit-form-panel" style="border-color: hsla(var(--success) / 0.3); background: hsla(var(--success) / 0.01);">
                    <h3 style="font-size: 1.25rem; margin-bottom: 1.5rem;"><i data-lucide="plus-circle"></i> Create New AI / SaaS Card</h3>
                    <form method="POST" action="admin.php?tab=services" class="edit-form-grid">
                        <input type="hidden" name="action" value="add_service">
                        
                        <div class="form-group">
                            <label for="title">Service Title</label>
                            <input type="text" name="title" class="form-control" placeholder="e.g. LLM Multi-Agent Framework" required>
                        </div>
                        ...
                        <div class="form-group">
                            <label for="icon">Lucide Icon Name</label>
                            <input type="text" name="icon" class="form-control" placeholder="brain" required>
                        </div>
                        ...
                        <div class="form-group full-width">
                            <label for="tech_stack">Technology Tags (Comma separated)</label>
                            <input type="text" name="tech_stack" class="form-control" placeholder="LangGraph, OpenAI, PyTorch" required>
                        </div>
            ...
            <div class="glass-card admin-card">
                <h3 style="font-size: 1.25rem; margin-bottom: 1rem;">AI / SaaS Services Catalog</h3>
                <div class="admin-table-container">
                    <table class="admin-table">
                        <thead>
                            <tr>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div style="display: flex; gap: 0.5rem;">
                                            ...
                                            <form method="POST" style="margin:0;" onsubmit="return confirm('Delete AI service?')">
        ...
        <?php elseif ($current_tab === 'pages'): ?>
            ...
            <?php if ($edit_page): ?>
                <div class="glass-card edit-form-panel">
                    <h3 style="font-size: 1.25rem; margin-bottom: 1.5rem;"><i data-lucide="edit"></i> Modify Lab Page: <?php echo htmlspecialchars($edit_page['title']); ?></h3>
            ...
            <?php else: ?>
                <div class="glass-card edit-form-panel" style="border-color: hsla(var(--success) / 0.3); background: hsla(var(--success) / 0.01);">
                    <h3 style="font-size: 1.25rem; margin-bottom: 1.5rem;"><i data-lucide="plus-circle"></i> Create New Lab Page</h3>
                    <form method="POST" action="admin.php?tab=pages" class="edit-form-grid">
                        <input type="hidden" name="action" value="add_page">
                        
                        <div class="form-group">
                            <label for="title">Page Title</label>
                            <input type="text" name="title" class="form-control" placeholder="e.g. Research Ethics" required>
                        </div>
                        
                        <div class="form-group">
                            <label for="slug">Page URL Slug (Unique, lowercase, e.g. research-ethics)</label>
                            <input type="text" name="slug" class="form-control" placeholder="research-ethics" required>
                        </div>
            ...
            <div class="glass-card admin-card">
                <h3 style="font-size: 1.25rem; margin-bottom: 1rem;">Dynamic Lab Pages List</h3>
                <div class="admin-table-container">
                    <table class="admin-table">
                        <thead>
                            <tr>
        ...
        <?php elseif ($current_tab === 'blogs'): ?>
            ...
            <?php if ($edit_blog): ?>
                <div class="glass-card edit-form-panel">
                    ...
                    <form method="POST" action="admin.php?tab=blogs" enctype="multipart/form-data" class="edit-form-grid">
                        ...
                        <div class="form-group">
                            <label for="slug">URL Slug (Unique, lowercase, e.g. voice-agent-latencies)</label>
                        ...
                        <div class="form-group full-width">
                            <label for="summary">Short Summary (Shows in list cards)</label>
                        ...
                        <div class="form-group full-width">
                            <label for="content">Publication body (HTML allowed)</label>
            ...
            <?php else: ?>
                <div class="glass-card edit-form-panel" style="border-color: hsla(var(--success) / 0.3); background: hsla(var(--success) / 0.01);">
                    <h3 style="font-size: 1.25rem; margin-bottom: 1.5rem;"><i data-lucide="plus-circle"></i> Create New Publication Log</h3>
                    <form method="POST" action="admin.php?tab=blogs" enctype="multipart/form-data" class="edit-form-grid">
                        <input type="hidden" name="action" value="add_blog">
                        
                        <div class="form-group">
                            <label for="title">Publication Title</label>
                            <input type="text" name="title" class="form-control" placeholder="e.g. Fine Tuning Quantized Models" required>
                        </div>
                        
                        <div class="form-group">
                            <label for="slug">URL Slug</label>
                            <input type="text" name="slug" class="form-control" placeholder="quantized-models-tuning" required>
                        </div>
                        ...
                        <div class="form-group full-width">
                            ...
                            <input type="text" name="summary" class="form-control" placeholder="Enter a brief summary..." required>
                        </div>
                        <div class="form-group full-width">
                            <label for="blog_image">Featured Image</label>
                            <input type="file" name="blog_image" id="blog_image" class="form-control" accept="image/*">
            ...
            <div class="glass-card admin-card">
                <h3 style="font-size: 1.25rem; margin-bottom: 1rem;">Research Publications List</h3>
                <div class="admin-table-container">
                    <table class="admin-table">
                        <thead>
                            <tr>
                                ...
                                <tr>
                                    ...
                                    <td>
                                        <div style="display: flex; gap: 0.5rem;">
                                            ...
                                            <form method="POST" style="margin:0;" onsubmit="return confirm('Delete publication?')">
        ...
        <?php elseif ($current_tab === 'settings'): ?>
            <div class="glass-card admin-card">
                <h3 style="font-size: 1.25rem; margin-bottom: 1.5rem;">AI Lab Global Settings</h3>
                <form method="POST" action="admin.php?tab=settings" enctype="multipart/form-data" class="edit-form-grid">
                    ...
                    <div class="form-group">
                        <label for="stats_projects">AI Projects Launched</label>
                    ...
                    <div class="form-group">
                        <label for="stats_students">Engineers Impacted</label>
                    ...
                    <div class="form-group">
                        <label for="stats_technologies">Active AI Stacks</label>
                    ...
                    <div class="form-group">
                        <label for="stats_experience">Research Focus</label>
                    ...
                    <div class="form-group">
                        <label for="logo_light_file">Light-Theme Logo (For Light Backgrounds)</label>
                        <input type="file" name="logo_light" id="logo_light_file" class="form-control" accept="image/*">
                    </div>

                    <div class="form-group">
                        <label for="logo_dark_file">Dark-Theme Logo (For Dark Backgrounds)</label>
                        <input type="file" name="logo_dark" id="logo_dark_file" class="form-control" accept="image/*">
                    </div>

                    <div class="form-group full-width">
                        <label for="logo_file">General/Login Page Logo</label>
        ...
        <?php elseif ($current_tab === 'industries'): ?>
            ...
            <?php if ($edit_industry): ?>
                <div class="glass-card edit-form-panel">
                    ...
                    <form method="POST" action="admin.php?tab=industries" class="edit-form-grid">
                        ...
                        <div class="full-width" style="margin-top: 1rem; display: flex; gap: 0.5rem;">
                            <button type="submit" class="btn btn-primary">Save Changes</button>
                            <a href="admin.php?tab=industries" class="btn btn-outline">Cancel</a>
                        </div>
                    </form>
                </div>
            <?php endif; ?>
                        ...
                        <tbody>
                            <?php foreach ($industries as $ind): ?>
                                <tr>
                                    ...
                                    <td>
                                        <div style="display: flex; gap: 0.5rem;">
                                            <a href="admin.php?tab=industries&edit_industry_id=<?php echo $ind['id']; ?>" class="action-icon-btn"><i data-lucide="edit" style="width: 16px; height: 16px;"></i></a>
        ...
        <?php elseif ($current_tab === 'reviews'): ?>
            ...
            <?php if ($edit_review): ?>
                <div class="glass-card edit-form-panel">
                    ...
                    <form method="POST" action="admin.php?tab=reviews" class="edit-form-grid">
                        ...
                        <div class="full-width" style="margin-top: 1rem; display: flex; gap: 0.5rem;">
                            ...
                            <a href="admin.php?tab=reviews" class="btn btn-outline">Cancel</a>
                        </div>
                    </form>
                </div>
            <?php endif; ?>
                        ...
                        <tbody>
                            <?php foreach ($reviews as $rev): ?>
                                <tr>
                                    ...
                                    <td>
                                        <div style="display: flex; gap: 0.5rem;">
                                            <a href="admin.php?tab=reviews&edit_review_id=<?php echo $rev['id']; ?>" class="action-icon-btn"><i data-lucide="edit" style="width: 16px; height: 16px;"></i></a>
        ...
        <?php elseif ($current_tab === 'users'): ?>
            ...
            <?php if ($edit_user): ?>
                <div class="glass-card edit-form-panel">
                    ...
                    <form method="POST" action="admin.php?tab=users" class="edit-form-grid">
                        ...
                        <tbody>
                            <?php foreach ($users as $usr): ?>
                                <tr>
                                    ...
                                    <td>
                                        <div style="display: flex; gap: 0.5rem;">
                                            <a href="admin.php?tab=users&edit_user_id=<?php echo $usr['id']; ?>" class="action-icon-btn"><i data-lucide="edit" style="width: 16px; height: 16px;"></i></a>
                                            <?php if ($usr['id'] !== (int)$_SESSION['lsxpl_admin_user_id']): ?>
        ...
        <?php elseif ($current_tab === 'account'): ?>
            ...
            <div class="glass-card edit-form-panel">
                ...
                <form method="POST" action="admin.php?tab=account" class="edit-form-grid" style="max-width: 600px;">
