<?php
// lsxpl/page.php - Dynamic CMS Page Layout
...
<head>
    ...
    <title><?php echo htmlspecialchars($page['title']); ?> | <?php echo htmlspecialchars($site['site_title'] ?? 'LSXPL'); ?></title>
    <meta name="description" content="<?php echo htmlspecialchars(substr(strip_tags($page['content']), 0, 160) ?: ($page['title'] . ' subpage of ' . ($site['site_title'] ?? 'LSPL'))); ?>">
    ...
    $canonical_href = '';
    if (!empty($site['canonical_url']) && !empty($page['slug'])) {
        $base = rtrim($site['canonical_url'], '/');
        $canonical_href = $base . '/page/' . $page['slug'];
    }
    ?>
