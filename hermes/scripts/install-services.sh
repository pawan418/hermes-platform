...
setup_filesystem_permissions() {
    ...
    for dir in "${dirs[@]}"; do
        ...
        # Adjust permissions to be writable by both owner and group (www-data)
        chmod -R 775 "$dir" >> "$LOG_FILE" 2>&1 || true
        
        # If www-data group exists, set group ownership
...
ensure_networks_and_volumes() {
    ...
    # Parse required network from docker-compose.yml (defaulting to hermes-network)
    local req_network="hermes-network"
    ensure_docker_network "$req_network"
